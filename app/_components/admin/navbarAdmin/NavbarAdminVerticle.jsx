...
//   { id: 6, name: "Whtsapp Inquiry", link: "/admin/whtsap" },
// ];
...
function NavbarAdminVerticle() {
  const pathname = usePathname();
  const [openSections, setOpenSections] = useState({}); // Track open state for each section
        ...
        {navData.map((el) => {
          ...
          if (el.isParent) {
            return (
              <li key={el.id}>
                <button
                  onClick={() => toggleSection(el.id)} // Toggle open state for this section
                  className={`flex items-center gap-3 font-medium px-4 py-2 rounded-lg transition w-full text-white ${
                    isActive ? "bg-blue-900" : ""
                  }`}
                >
